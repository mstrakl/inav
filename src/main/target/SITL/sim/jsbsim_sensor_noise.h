/*
 * Simple sensor noise/bias/lag simulator for JSB SITL
 * Intended to be included only by SITL sim code (C usage)
 *
 * Usage:
 *  - call `jsbSimNoiseInit()` once at sim startup
 *  - call `jsbSimApplySensorNoise(...)` periodically from the sim update/listen worker
 *
...
#ifndef JSBSIM_SENSOR_NOISE_H
#define JSBSIM_SENSOR_NOISE_H
...
static inline float jsbSimGaussianNoise(void)
{
...
static inline int32_t jsbSimMetersTo1e7Lat(float meters)
{
...
static inline int32_t jsbSimMetersTo1e7Lon(float meters_east, int32_t lat_1e7)
{
...
static jsbSimVecState_t s_accelState = {{0,0,0},{0,0,0}};
static jsbSimVecState_t s_gyroState = {{0,0,0},{0,0,0}};
static jsbSimVecState_t s_magState = {{0,0,0},{0,0,0}};
static jsbSimScalarState_t s_rollState = {0,0};
static jsbSimScalarState_t s_pitchState = {0,0};
static jsbSimScalarState_t s_gsState = {0,0};
static jsbSimScalarState_t s_hpathState = {0,0};
static int s_seeded = 0;

// Initialize RNG and optional initial bias
static inline void jsbSimNoiseInit(void)
{
    if (!s_seeded) {
...
static inline float jsbSimApplySignalNoise(float raw, float noiseStd, float biasWalkStd, float tau, float *biasPtr, float *lagPtr, float dt)
{
    // bias random walk
    if (biasWalkStd > 0.0f) {
        // scale random walk by sqrt(dt) to make it independent of update rate
        float step = jsbSimGaussianNoise() * biasWalkStd * sqrtf(dt);
        *biasPtr += step;
    }

    float noise = (noiseStd > 0.0f) ? jsbSimGaussianNoise() * noiseStd : 0.0f;
...
static inline void jsbSimApplySensorNoise(
    ...
    float dt)
{
    ...
    for (int i=0;i<3;i++) {
        a[i] = jsbSimApplySignalNoise(a[i], DEFAULT_ACCEL_NOISE_STD_MS2, DEFAULT_ACCEL_BIAS_WALK_STD_MS2, DEFAULT_ACCEL_LAG_TAU_S, &s_accelState.bias[i], &s_accelState.lagState[i], dt);
    }
    *accel_x = a[0]; *accel_y = a[1]; *accel_z = a[2];
    ...
    for (int i=0;i<3;i++) {
        g[i] = jsbSimApplySignalNoise(g[i], DEFAULT_GYRO_NOISE_STD_DPS, DEFAULT_GYRO_BIAS_WALK_STD_DPS, DEFAULT_GYRO_LAG_TAU_S, &s_gyroState.bias[i], &s_gyroState.lagState[i], dt);
    }
    *gyro_x = g[0]; *gyro_y = g[1]; *gyro_z = g[2];

    // roll / pitch (angles)
    *roll = jsbSimApplySignalNoise(*roll, DEFAULT_ANGLE_NOISE_STD_DEG, DEFAULT_ANGLE_BIAS_WALK_STD_DEG, DEFAULT_ANGLE_LAG_TAU_S, &s_rollState.bias, &s_rollState.lagState, dt);
    *pitch = jsbSimApplySignalNoise(*pitch, DEFAULT_ANGLE_NOISE_STD_DEG, DEFAULT_ANGLE_BIAS_WALK_STD_DEG, DEFAULT_ANGLE_LAG_TAU_S, &s_pitchState.bias, &s_pitchState.lagState, dt);

    // magnetometer (north vector sample)
    float m[3] = {*mag_x, *mag_y, *mag_z};
    for (int i=0;i<3;i++) {
        m[i] = jsbSimApplySignalNoise(m[i], DEFAULT_MAG_NOISE_STD, DEFAULT_MAG_BIAS_WALK_STD, DEFAULT_MAG_LAG_TAU_S, &s_magState.bias[i], &s_magState.lagState[i], dt);
    }
    *mag_x = m[0]; *mag_y = m[1]; *mag_z = m[2];
    ...
    float northNoiseM = jsbSimGaussianNoise() * DEFAULT_GPS_POS_NOISE_M;
    float eastNoiseM  = jsbSimGaussianNoise() * DEFAULT_GPS_POS_NOISE_M;
    ...
    if (DEFAULT_GPS_POS_BIAS_WALK_M > 0.0f) {
        s_gpsBiasNorth += jsbSimGaussianNoise() * DEFAULT_GPS_POS_BIAS_WALK_M * sqrtf(dt);
        s_gpsBiasEast  += jsbSimGaussianNoise() * DEFAULT_GPS_POS_BIAS_WALK_M * sqrtf(dt);
    }
    ...
    if (lat_1e7 && lon_1e7) {
        int32_t dlat = jsbSimMetersTo1e7Lat(s_gpsLagNorth);
        int32_t dlon = jsbSimMetersTo1e7Lon(s_gpsLagEast, *lat_1e7);
        *lat_1e7 = *lat_1e7 + dlat;
        *lon_1e7 = *lon_1e7 + dlon;
    }

    // groundspeed (m/s)
    *groundspeed = jsbSimApplySignalNoise(*groundspeed, DEFAULT_GS_NOISE_MPS, DEFAULT_GS_BIAS_WALK_MPS, DEFAULT_GS_LAG_TAU_S, &s_gsState.bias, &s_gsState.lagState, dt);

    // hpath/track (degrees)
    *hpath = jsbSimApplySignalNoise(*hpath, DEFAULT_HPATH_NOISE_DEG, DEFAULT_HPATH_BIAS_WALK_DEG, DEFAULT_HPATH_LAG_TAU_S, &s_hpathState.bias, &s_hpathState.lagState, dt);
}

#endif // JSBSIM_SENSOR_NOISE_H
